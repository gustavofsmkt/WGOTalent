...
> Fechamento do marco greenfield (TASK-130), atualizado após três decisões
> de produto aditivas que entraram depois do congelamento: captação de
> currículo por e-mail
> ([ADR-0010](decisions/0010-captacao-curriculo-via-email.md)), OpenAI como
> segundo provedor de LLM
> ([ADR-0011](decisions/0011-multiplos-provedores-llm.md)) e banco de
> talentos automático
> ([ADR-0013](decisions/0013-banco-de-talentos-automatico.md)). Consolida em
...
- Extração, classificação e avaliação de candidatos via motor de agentes de
  ...
  `/admin/credenciais`, com 2 provedores de LLM implementados — Gemini e
  OpenAI, por trás de um dispatcher comum (ADR-0011).
- Captação automática de currículo por e-mail via IMAP genérico (loop interno
  ...
  (ADR-0010).
- Banco de talentos automático: candidato sem vaga compatível no momento do
...
20 migrations aplicadas (`drizzle/0000`…`0019`), cobrindo criação de todas as
...
candidato para nullable+unique, a tabela de credenciais de e-mail e a coluna
...
- **Motor de IA** — pipeline nativo `extração → classificação → avaliação`
  (`src/server/agents/`), configurável por slot em `/admin/agentes`, com 2
  provedores de LLM implementados por trás de um dispatcher comum
  (`src/lib/agents/agent-client.ts`): Gemini (`@google/genai`) e OpenAI
  (Responses API via `fetch` nativo, sem dependência nova) — ADR-0011.
  Candidato sem par aprovado nessa pipeline entra automaticamente no banco
  de talentos (`candidatos.em_banco_talentos`, ADR-0013). Credenciais de
  provedor de LLM e de e-mail cifradas em repouso (AES-256-GCM,
  `src/lib/agents/crypto.ts`) e nunca expostas fora da camada de repository
  (ADR-0007, ADR-0010, ADR-0011, SECURITY.md).

## Testes

- 56 arquivos de teste / 456 testes (Vitest), todos verdes após a adição da
  captação por e-mail, do segundo provedor de LLM e do banco de talentos
  automático (48 arquivos / 390 testes na validação final original do
  greenfield).
- Cobertura em repositories, Server Actions, validação Zod, motor de
  agentes (extração/classificação/avaliação/orquestração, Gemini e OpenAI),
  storage, cripto de credenciais, cliente IMAP, ciclo/loop de captação de
  e-mail e rota de arquivos.

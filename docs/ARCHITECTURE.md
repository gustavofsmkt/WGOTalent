...
- **Database**: PostgreSQL
- **ORM**: Drizzle
- **Auth**: identidade local, senha com `scrypt` e sessão stateless em cookie assinado; sem autorização/RBAC (ADR-0012)
...
src/
  app/
    ...
    api/
      files/
        [...path]/route.ts         # GET — streams resume files from local storage (authentication required)
...
5. **Catalog integrity in the UI.** Cargo form selects an existing Departamento. Vaga form selects an existing Cargo. Triagem form selects an existing Candidato and Vaga. Candidato's `cargo_interesse` / `area_interesse` are optional text fields populated by the extraction agent; manual create/edit uses server-fetched active catalog options and persists their names, never their IDs. Cargo determines its corresponding area; area never determines a cargo automatically.
...
7. **No premature complexity.** Skip parallel routes, intercepting routes/modals, and authorization/RBAC for this MVP pass — plain nested pages with Suspense boundaries around any slower list (e.g. triagens with joins across candidato/vaga/avaliacao_ia) is enough.

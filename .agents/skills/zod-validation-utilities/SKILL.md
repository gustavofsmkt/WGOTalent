...
- Integrating Zod v4 schemas with TanStack Form via Standard Schema (`~standard`)
...
### 6) Error handling workflow with `safeParse`
...
> **Tip**: For advanced discriminated union patterns, see `references/advanced-patterns.md`. For TanStack Form integration, pass Zod v4 schemas directly to `validators` — Zod v4 implements Standard Schema (`~standard`) natively, no adapter needed.

...
TanStack Form is a headless form library with deep TypeScript integration. It provides field-level and form-level validation (sync/async), array fields, linked/dependent fields, fine-grained reactivity, and native Standard Schema support (Zod v4 implements `~standard` natively — no adapter needed).
...
### Schema Validation (Standard Schema / Zod v4)

Zod v4 implements the Standard Schema protocol (`~standard`) natively. Pass a Zod schema directly to `validators` — no adapter or extra package needed.
...
<form.Field
  name="email"
  validators={{
    onChange: z.string().email({ error: 'E-mail inválido' }),
    onBlur: z.string().min(1, { error: 'Campo obrigatório' }),
  }}
/>
...
<form.Field
  name="age"
  validators={{
    onChange: z.number().min(18, { error: 'Mínimo 18 anos' }),
  }}
/>
```

For form-level schemas (e.g. cross-field rules), pass the schema to `validators.onChange` at the `useForm` level:
...
Server-side validation in this project uses plain Server Actions + Zod `.safeParse()`. Do **not** use `ServerValidateError` from `@tanstack/react-form/nextjs` — that package is not installed and not needed.

```ts
// src/actions/departamentos.ts
'use server'
import { criarDepartamentoSchema } from '@/lib/validation/departamento'
...
The client form reads `result.error` and maps it back to field errors using TanStack Form's `form.setFieldMeta` or by rendering the returned error object directly.

...
## 1. Motor de Agentes Nativo (`@google/genai`)

- **IA Interna:** Chamadas de IA são internas à aplicação via `src/lib/agents/gemini-client.ts` (SDK `@google/genai`, provedor Gemini/Google AI Studio) e **NÃO** requerem shared secret de webhook, eliminando a antiga arquitetura via n8n (conforme decidido no **ADR-0007**).
